...
import { toggleQueuedDelay, enqueueBlockConfigChange } from '../../services/CtblApiClient.js';
...
export var LockEditor = {
  ...
  _applyLockedRestrictions: function (p) {
    var isQueuedDelay = isExtensionLock(settings.blocks[this.blockName], "queuedDelay")
      && AppState.configuredQueuedDelays && AppState.configuredQueuedDelays.indexOf(this.blockName) > -1;

    if (this.isLocked != "false" && !isQueuedDelay) {
  ...
  _onSave: function () {
    ...
    // ── Queued Delay intercept: route lock-config changes through the queue ──
    var isCurrentlyQD = isExtensionLock(settings.blocks[blockName], "queuedDelay")
    ...
    var isFrozenContinuous = blockName.indexOf("Frozen Turkey") === 0
      && settings.blocks[blockName].type === "continuous";

    if (isCurrentlyQD && selectedLockType !== "queuedDelay" && !isFrozenContinuous) {
      enqueueBlockConfigChange(blockName, {
        lock: lockConfig,
        password: passwordConfig,
        randomTextLength: randomTextConfig,
        window: windowConfig,
        lockUnblock: $("#lock_unblock").is(":checked").toString(),
        restartUnblock: $("#restart_unblock").is(":checked").toString()
      });
      $dlg.dialog("close");
      return;
    }
