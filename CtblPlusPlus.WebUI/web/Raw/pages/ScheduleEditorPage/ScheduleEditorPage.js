...
import { enqueueScheduleChange } from '../../services/CtblApiClient.js';
...
export function editSchedule(blockId, isLockedStr) {
  ...
  $dialogEditSchedule.dialog({
    ...
    open: function () {
      ...
      var isQueuedDelay = isExtensionLock(settings.blocks[blockId], "queuedDelay")
        && AppState.configuredQueuedDelays && AppState.configuredQueuedDelays.indexOf(blockId) > -1;
      if (isLockedStr != "false" && !isQueuedDelay) {
    ...
    buttons: {
      ...
      Save: {
        ...
        click: function () {
          ...
          var newType = $("#edit-schedule-continuous").is(":checked") ? "continuous" : "scheduled";

          // ── Queued Delay intercept ──
          var isQDSave = isExtensionLock(settings.blocks[blockId], "queuedDelay")
            && AppState.configuredQueuedDelays && AppState.configuredQueuedDelays.indexOf(blockId) > -1
            && settings.blocks[blockId].enabled === "true";
          var isFrozenContinuous = blockId.indexOf("Frozen Turkey") === 0
            && settings.blocks[blockId].type === "continuous";

          if (isQDSave && !isFrozenContinuous) {
            var schedPayload = {
              type: newType,
              schedule: AppState.schedule.slice(),
              autostart: settings.blocks[blockId].autostart
            };
            if (isFrozenTurkey && newType == "continuous") {
              var nowQD = new Date();
              schedPayload.startTime = nowQD.getFullYear().toString() + "," + (nowQD.getMonth() + 1).toString() + "," + nowQD.getDate().toString() + "," + nowQD.getHours().toString() + "," + nowQD.getMinutes().toString();
              schedPayload.enabled = "false";
            }
            enqueueScheduleChange(blockId, schedPayload);
            $dialogEditSchedule.dialog("close");
            return;
          }

          settings.blocks[blockId].type = newType;
