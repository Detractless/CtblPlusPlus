...
import { updateBrowserList, showExtensionInstall } from './components/ExtensionInstallModal/ExtensionInstallModal';
import { passwordCheck } from './components/SecurityModal/SecurityModal';
import { stats, exportStats } from './components/StatsChart/StatsChart';
import { addRemovePayWallEvents, showTrialModal, showHideUpdateButton, showUpgradeModal, startTrial, upgradeResult } from './components/TrialModal/TrialModal';
import { deleteStats } from './components/StatsToggle/StatsToggle';
import { showAddNetworks } from './components/NetworkListEditor/NetworkListEditor';
...
window.showExtensionInstall = showExtensionInstall;
window.exportStats = exportStats;
window.deleteStats = deleteStats;
window.showAddNetworks = showAddNetworks;
window.startTrial = startTrial;
window.upgradeResult = upgradeResult;
...
jQuery(document).ready(function() {
    if (typeof window.settings === 'undefined' || !window.settings) {
        try {
            ...
            // Cold Turkey will push settings via ForceSettingsUpdate() once ready
        }
    }
