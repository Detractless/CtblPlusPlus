...
export function calculateBlockState(blockName, blockData, currentDate, unlockedBlocksArray) {
  ...
  if (blockData.type == "continuous") {
    ...
    if (isFrozen) {
      var frozenIsQD = findLockType(blockName, blockData) !== null;
      ...
      if (moment(currentDate).isBefore(timerDate)) {
        disabledAttribute = (lockStrictness >= 1 && !frozenIsQD) ? " disabled=\"disabled\"" : "";
        lockTypeData = (lockStrictness >= 1 && !frozenIsQD) ? "true" : "false";
