...
import { cancelQueuedDelay, enqueueListAction, cancelListAction, cancelBlockConfigChange, cancelScheduleChange } from '../../services/CtblApiClient';
...
export function flushPendingQueueOps() {
  ...
  ops.forEach(function (op) {
    ...
    } else if (op.type === 'cancel-block-config') {
      promises.push(cancelBlockConfigChange(op.blockName));
    } else if (op.type === 'cancel-schedule-change') {
      promises.push(cancelScheduleChange(op.blockName));
    }
  });
  ...
  if (cleanedUrl != "" && cleanedUrl != "null") {
    if ($("#blocklist-" + listType + "-list div[data-value=\"" + cleanedUrl.replace(/'/g, "\\'").replace(/"/g, "\\\"") + "\"]").length < 1) {
      if (listType == "exceptions") {
        ...
        if (hasConflict) {
          var conflictBlockName = $("#dialog-edit-block").attr("data-saveTitle");
          var conflictLock = conflictBlockName && settings.blocks[conflictBlockName] ? findLockType(conflictBlockName, settings.blocks[conflictBlockName]) : null;
          if (conflictLock && conflictLock.id === "queuedDelay" && settings.blocks[conflictBlockName].enabled === "true") {
            // Let the exception fall through to the queue logic below instead of blocking it
          } else {
          if (!isBatch) {
      ...
      if (listType === "exceptions") {
        ...
        if (blockName && settings.blocks[blockName]) {
          ...
          if (matchedLock && matchedLock.id === "queuedDelay" && settings.blocks[blockName].enabled === "true") {
...
export function removeEntry(listType) {
  ...
  if ($("#blocklist-" + listType + "-list div.custom-option.selected").length > 0) {
    if (listType === "queue") {
      ...
      var blockConfigEntries = serverEntries.filter('[data-entry-type="block-config"]');
      if (blockConfigEntries.length > 0) {
        pendingQueueOps.push({ id: nextPendingId++, type: 'cancel-block-config', blockName: blockName });
        blockConfigEntries.remove();
      }
      var scheduleChangeEntries = serverEntries.filter('[data-entry-type="schedule-change"]');
      if (scheduleChangeEntries.length > 0) {
        pendingQueueOps.push({ id: nextPendingId++, type: 'cancel-schedule-change', blockName: blockName });
        scheduleChangeEntries.remove();
      }
