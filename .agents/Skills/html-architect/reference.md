...
This file is loaded by phase files that need the structural methodology — primarily `direct.md` for placement decisions and semantic checks, and occasionally `part-auto.md` for verification context. It is never loaded directly by `SKILL.md` and never loaded for Auto phase (scripts don't need principles).
