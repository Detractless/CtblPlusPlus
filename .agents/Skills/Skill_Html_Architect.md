---
name: html-architect
description: Decides where code belongs and why. Use when creating new files, folders, or modules. Use when something feels like it's in the wrong place. Use when a codebase is growing and structure is starting to drift. Use when a new developer needs to understand how the project is organized.
---
...
Determine where code belongs, how modules should be divided, and where layer boundaries should be drawn — without writing any implementation. The goal is not a perfect folder structure on day one — it's a structure that stays coherent as the project grows. Every structural decision must pass a single test: "If I deleted this folder entirely, would only its direct parent break — or would ten other things break too?"

## When to Use

- Before creating any new file, to decide where it belongs and what it should be named
- When a file has grown large and needs to be split into focused modules
- When logic from one layer is bleeding into another (business logic in components, DOM manipulation in services)
- When a new developer asks "where do I put X?" and the answer is unclear
- When two files are doing overlapping things and neither is clearly the right owner
- After a fast-paced feature sprint that may have broken structural discipline
- When setting up a new project and choosing the initial folder layout

**When NOT to use:**

- You haven't read the existing project structure yet — map before deciding
- The codebase is a single throwaway script — flat structure is correct for trivial scope
- A structural decision requires understanding runtime behavior — this skill is about static organization, not execution
- You're mid-feature and the change would require restructuring more than 3 files — finish the feature first, restructure after
- The question is about naming conventions inside a file, not file placement — that belongs to the Naming Reviewer
