# CTBL++ Unified Findings

**Merged from:** code-review.md · code-review-findings.md · frontend-code-review.md · frontend-ui-review.md · build-system-review.md · ci-build-review-2026-07-05.md · cross-component-review.md · holistic-review-2026-07-05.md · block-type-system-review.md · agent-skill-review.md · skill-library-review.md · security-review-findings (opus 4.8)  
**Date of source reviews:** 2026-07-05  
**Last updated:** 2026-07-06 — seven new security findings (CS-51 through CS-57) and trust-boundary detail added from secondary deep-trace review.
...
**CS-51 — `%ProgramData%\CtblPlusPlus\` root is never hardened — `system.key` and `ctblplusplus.db` fully exposed to admin writes**  
**Files:** `PersistenceEnforcer.cs:72–88`; `VaultAclEnforcementService.cs:51–64`; `DpapiHmacProvider.cs:17–22`; `SqliteBaseRepository.cs:13–18`  
`system.key` and `ctblplusplus.db` both reside in `%ProgramData%\CtblPlusPlus\`. Neither of the two ACL-enforcement services hardens this root directory: `PersistenceEnforcer` hardens `installDir`, `vault`, and `%ProgramData%\Cold Turkey` (delete-guard only); `VaultAclEnforcementService` hardens `installDir` and `vault`. Default `%ProgramData%` inheritance grants `Users` read on both files and `Administrators` full control. An administrator can therefore write `ctblplusplus.db` directly, which is the exact threat model the queue HMAC is supposed to resist. `Anti-TM.txt` (unsigned plaintext ticks, `TimeEnforcer.cs:42`) also lives in this un-hardened root. Fix: apply `HardenVault`-style ACLs to `%ProgramData%\CtblPlusPlus\` itself, or at minimum apply `DenyAdminWrite` to `system.key`, `ctblplusplus.db`, and `Anti-TM.txt` individually.  
*Source: security-review-findings (opus 4.8) — Finding A1. Not present in prior sweepers.*

**CS-52 — DPAPI key confidentiality never enforced — `system.key` is readable by any local user, not just admins**  
**Files:** `DpapiHmacProvider.cs:53, 79`; `AclHelper.cs:27–111`  
`system.key` is protected with `DataProtectionScope.LocalMachine`, which means any local process — regardless of privilege level — can call `ProtectedData.Unprotect()` on the file's bytes once it has read them. `HardenVault` and `DenyAdminWrite` deny *write* access but never add a read-deny ACE for `Administrators`, `Users`, or any other SID (see `AclHelper.cs:27–111` — only `FileSystemRights.Write | Delete | ChangePermissions | TakeOwnership` are denied; `Read` and `Execute` are untouched). The key that signs every queue entry and every watchdog pipe message is therefore readable by the adversary this tool exists to constrain. This is distinct from Known CC-15 (which frames the issue as DPAPI-scope breadth and "admin only"); the actionable point here is the absent file read-deny ACE and the reach of `LocalMachine` DPAPI to any local process. Fix: add an explicit `FileSystemRights.Read` deny rule for `Administrators` and `Users` on `system.key` in `AclHelper`, and evaluate switching to `DataProtectionScope.CurrentUser` under the service account identity.  
*Source: security-review-findings (opus 4.8) — Finding A2. Not present in prior sweepers.*

**CS-53 — `BackfillKinds()` is a signing oracle — attacker-inserted DB rows receive valid HMAC signatures on Engine restart, bypassing all queue integrity without the key**  
**File:** `CtblPlusPlus.Infrastructure/Persistence/Repositories/SqliteQueueRepository.cs:33–88`  
`BackfillKinds()` runs in the repository constructor (executed on every Engine start). It selects every row `WHERE Kind = '' OR Kind IS NULL`, calls `Classify()` to re-derive the kind, then computes a fresh V2 HMAC signature via `ComputeHmac(BuildV2(row))` and writes it back with `UPDATE … SET Signature=…`. Critically, it never reads or verifies the row's existing `Signature` column. This creates a complete exploitation path:  
1. Adversary (admin, possible via CS-51) INSERTs a row directly into `ctblplusplus.db`: `Kind=''`, `Status='Pending'`, `UnlockAt=<past timestamp>`, `BlockName='<a real locked block>'`, `TargetUrl='CTBL_QUEUED_DELAY'`, `Signature='anything'`.  
2. Engine restarts (frequent, given watchdog resurrection). Repository constructor runs `BackfillKinds()` → `Classify()` → `ComputeHmac(BuildV2(row))` → `UPDATE … SET Signature=<valid>`. The forged row now carries a cryptographically valid signature.  
3. `MigrateSignaturesToV2()` (line 137) sees `Signature == v2 prefix` → skips migration, row passes.  
4. `QueueDispatcher.ProcessQueueAsync:96` — `VerifyHmac` passes → `QueuedDelayQueueHandler` unlocks the block.  
The entire queued delay — the core security feature — is bypassed with no key required. The re-signing is load-bearing (it is what allows `MigrateSignaturesToV2` to skip the row), so it cannot simply be deleted. Fix: in `BackfillKinds`, `SELECT Signature` along with the other fields; verify the existing signature against the legacy V1 formula (`BuildV1` with empty `Kind`) before re-signing; set `Status = 'FailedSecurityViolation'` on mismatch rather than re-signing.  
*Source: security-review-findings (opus 4.8) — Finding A3. Not present in prior sweepers.*

**CS-54 — App Control allow/disable bypasses the delay queue entirely and is unauthenticated — reachable by any local process or web page**  
**Files:** `CtblApiClient.js allowApp/disableAppControl`; `AppControlHandler.cs:47–62`; `AppControlQueueHandler.cs:39, 124`  
Every other website/block-config/schedule/global-delay mutation is delay-gated server-side: `UnlockAt` is computed from `GlobalDelayHours` in `QueuedDelayService` and the client cannot backdate it. App Control is the exception. The path is: `CtblApiClient.js allowApp/disableAppControl` → `GET /api/app-control?action=allow|disable` → `AppControlHandler.cs:47–62` → `AppControlQueueHandler.HandleEnable` (line 39: sets `Allowed + RemoveApps` immediately) or `HandleDisable` (line 124: sets `AppControlEnabled=false` and resets every blocked app). There is no delay, no lock check, and no authentication of any kind. That this is an oversight rather than intent is confirmed by: (1) `EnforcerSettingsHandler.cs:70` explicitly refuses to disable an enforcer while any block is locked via `IsAnyBlockLocked()`; (2) the `AppAllow` and `AppDisableControl` kinds already exist in `QueueRequestKinds` for a delayed path that is never wired up. Because the HTTP API carries `Access-Control-Allow-Origin: *` (CC-01) and the action is a GET, any web page open while the Engine runs can call `GET http://127.0.0.1:58123/api/app-control?action=disable` and instantly disable all app enforcement with no user interaction. Fix: route `allow` and `disable` through the delay queue using the existing `AppAllow`/`AppDisableControl` kinds, or at minimum apply the `IsAnyBlockLocked` guard present in `EnforcerSettingsHandler`.  
*Source: security-review-findings (opus 4.8) — Finding B. Not present in prior sweepers.*

### Medium Severity

**CS-55 — `AppControlEnabled` has two uncoordinated writers — state self-disables within 45 seconds of a user enabling it**  
**Files:** `CtblStateEnforcer.cs:55–58`; `ColdTurkeyInjector.cs:219`; `AppControlStateManager.cs:36`  
`CtblStateEnforcer.ExecuteAsync` overwrites `AppControlEnabled` every 45 seconds by calling `ReadBlockEnabled()` on the App Control whitelist block. `EnsureBlockExists` creates that whitelist block with `Enabled="false"` (`ColdTurkeyInjector.cs:219`). No C# path in the codebase sets the block's `Enabled` flag to `"true"` (confirmed by grep). Meanwhile the HTTP API's `HandleEnable` writes `AppControlEnabled=true` directly to the settings store. The result: (1) user enables App Control → `AppControlEnabled=true`, apps injected; (2) within 45 s, `CtblStateEnforcer` reads `block.Enabled="false"` → overwrites `AppControlEnabled=false`; (3) `SyncDetectedApps` gate in `AppControlStateManager.cs:36` returns early; `GetAppControlData` reports `enabled:false` to a UI that believes it just enabled the feature. App Control silently self-disables and there is no error surfaced. Fix: choose one source of truth — either have `HandleEnable` call `StartBlock` on the whitelist block and derive `AppControlEnabled` solely from the block flag, or stop `CtblStateEnforcer` from overwriting an explicit enable.  
*Source: security-review-findings (opus 4.8) — Finding D. Not present in prior sweepers.*
...
**CS-56 — Fail-open HMAC: `ComputeHmac` returns `""` when key is not loaded — empty signature is silently accepted**  
**Files:** `DpapiHmacProvider.cs:90`; `PidBroker.cs:112`  
`DpapiHmacProvider.ComputeHmac` returns `""` (the empty string) when `_hmacKey` is null or empty (line 90 — the key is not yet loaded or was not initialized). `PidBroker.cs:112` verifies pipe messages with `ComputeHmac(payload) != providedSig`. If an attacker sends a message where `providedSig == ""` while the key is in the empty state, `"" != ""` evaluates to `false` → the verification passes and the message is accepted as authentic. The empty-key state is reachable via the mutex-timeout fall-through documented in CS-05: if `WaitOne(30s)` times out, `_hmacKey` may never be populated before `ComputeHmac` is first called. Fix: throw an `InvalidOperationException` (or return a never-matching sentinel that cannot be sent over the wire, such as `null`) when `_hmacKey` is empty rather than returning `""`.  
*Source: security-review-findings (opus 4.8) — Secondary latent bug. Not present in prior sweepers.*
...
**CS-57 — `ScheduleChangeQueueHandler`: switching a schedule-locked block to continuous silently clears the lock — surprising side-effect with no documentation**  
**File:** `CtblPlusPlus.Application/Queue/Handlers/ScheduleChangeQueueHandler.cs:87–93`  
When a block's scheduling type is changed from `"scheduled"` to `"continuous"`, the handler sets `block.Lock = "none"` if the current value is a schedule lock. This means a schedule-locked block can be reduced to a lockless state via a single queued type-change request. The request is delay-gated (it goes through the normal queue), so this is not a straight bypass — the configured delay applies. However, the behavior is surprising: a user who has set a schedule lock expecting it to be durable finds it silently cleared as a side-effect of a scheduling type change, with no warning in the UI and no documentation of this behavior. Fix: document this as an intentional side-effect in the handler via a comment; surface a warning in the UI before the change is enqueued.  
*Source: security-review-findings (opus 4.8) — Secondary latent bug. Not present in prior sweepers.*
...
**CC-01 — CORS wildcard + JSONP on the enforcement API create a complete bypass vector; trust boundary is drawn at the wrong abstraction level**  
**Files:** `LocalWebServerService.cs:72–78, 145–162`; `PidBroker.cs:112, 118–138`  
...
**Additional trust-boundary detail (secondary review):** The Engine has two control planes over the same enforcement state with opposite trust assumptions. The named pipe (`CtblPlusPlusPidBroker`) requires every message to be HMAC-signed and verified (`PidBroker.cs:112`, `WatchdogHeartbeat.cs:68`). The HTTP API at port 58123 carries no signature, no bearer token, and nothing. Fixing CORS/JSONP closes the browser vector but does not close the local non-browser-process vector: any process running on the machine retains full unauthenticated access to enqueue requests, cancel entries, and (per CS-54) instantly disable App Control. Furthermore, because of CS-52, the pipe's HMAC key is itself readable by any local user, making the pipe's HMAC forgeable by any local process that reads `system.key`. The practical trust boundary the system enforces is therefore "any local process" — for both control planes simultaneously — not "signed callers only." The pipe also trusts the PID value inside `REGISTER_WATCHDOG` (`PidBroker.cs:118–138`) without verifying that the connecting process is actually that binary, so a key-holding local process can register a bogus PID or spoof `HEARTBEAT|Wd1` to suppress resurrection. Fix (after CS-51/CS-52 are addressed): sign API mutations with the same HMAC key (or issue a per-boot nonce to the WebView2 host object and require it on mutating routes).  
*Source: ci-build-review-2026-07-05.md (SEC-01, SEC-02) · build-system-review.md (§9) · code-review.md (8.5) · code-review-findings.md (#3) · cross-component-review.md (§3 Medium) · security-review-findings (opus 4.8, Finding C). Flagged by six sweepers.*
...
| C# Backend | 57 (50 original + CS-51–CS-57 from secondary review) |
| HTML / JS Frontend | 50 |
| Build / CI / Scripting | 23 |
| Cross-Cutting | 19 (CC-01 expanded with trust-boundary detail) |
| Block Type Architecture | 11 |
| Agent Skills (.agents) | 25 |
| **Total in merged output** | **197** |

The raw count (235) from the first pass exceeded the merged count (190) primarily due to deduplication. The secondary deep-trace review (opus 4.8) added 7 genuinely new entries not present in any prior sweeper: CS-51 (un-hardened %ProgramData%\CtblPlusPlus\ root), CS-52 (DPAPI key read-deny absent), CS-53 (BackfillKinds signing oracle), CS-54 (App Control unauthenticated bypass), CS-55 (AppControlEnabled two writers), CS-56 (fail-open HMAC on empty key), CS-57 (schedule→continuous silently clears lock). CC-01 was expanded with detail on the asymmetric trust boundary between the pipe and HTTP control planes.
