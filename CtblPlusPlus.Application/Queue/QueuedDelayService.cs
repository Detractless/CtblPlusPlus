...
public class QueuedDelayService
{
    ...
    public void EnqueueBlockConfigChange(string blockName, string payloadJson)
    {
        // Replace-not-stack: cancel any existing pending config change for this block
        var existing = _queueRepo.GetPendingRequests()
            .Where(r => r.BlockName == blockName && r.Kind == QueueRequestKinds.BlockConfigChange)
            .ToList();
        foreach (var old in existing)
            _queueRepo.UpdateRequestStatus(old.Id, QueueStatus.Rejected);
        ...
        _queueRepo.AddRequest(new DelayRequest
        {
            Id = Guid.NewGuid().ToString("N"),
            BlockName = blockName,
            TargetUrl = QueueCommandPrefixes.BlockConfig + payloadJson,
            Kind = QueueRequestKinds.BlockConfigChange,
            RequestedAt = DateTime.UtcNow,
            UnlockAt = DateTime.UtcNow.AddHours(delayHours),
            Status = QueueStatus.Pending
        });
    ...
    public CancelResult CancelBlockConfigChange(string blockName)
    {
        var existing = _queueRepo.GetPendingRequests()
            .Where(r => r.BlockName == blockName && r.Kind == QueueRequestKinds.BlockConfigChange)
            .ToList();
    ...
    public DelayRequest? GetPendingBlockConfigChange(string blockName)
    {
        return _queueRepo.GetPendingRequests()
            .FirstOrDefault(r => r.BlockName == blockName && r.Kind == QueueRequestKinds.BlockConfigChange);
    }

    // ── Schedule Change ─────────────────────────────────────────────────────

    public void EnqueueScheduleChange(string blockName, string payloadJson)
    {
        var existing = _queueRepo.GetPendingRequests()
            .Where(r => r.BlockName == blockName && r.Kind == QueueRequestKinds.ScheduleChange)
            .ToList();
        foreach (var old in existing)
            _queueRepo.UpdateRequestStatus(old.Id, QueueStatus.Rejected);
        ...
        _queueRepo.AddRequest(new DelayRequest
        {
            Id = Guid.NewGuid().ToString("N"),
            BlockName = blockName,
            TargetUrl = QueueCommandPrefixes.ScheduleChange + payloadJson,
            Kind = QueueRequestKinds.ScheduleChange,
            RequestedAt = DateTime.UtcNow,
            UnlockAt = DateTime.UtcNow.AddHours(delayHours),
            Status = QueueStatus.Pending
        });
    ...
    public CancelResult CancelScheduleChange(string blockName)
    {
        var existing = _queueRepo.GetPendingRequests()
            .Where(r => r.BlockName == blockName && r.Kind == QueueRequestKinds.ScheduleChange)
            .ToList();
    ...
    public DelayRequest? GetPendingScheduleChange(string blockName)
    {
        return _queueRepo.GetPendingRequests()
            .FirstOrDefault(r => r.BlockName == blockName && r.Kind == QueueRequestKinds.ScheduleChange);
    }
